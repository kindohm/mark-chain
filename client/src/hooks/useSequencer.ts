...
const OSC_CONFIG_STORAGE_KEY = "brunchh-osc-config";
  ...
  try {
    ...
    return {
      ...(typeof parsed.enabled === "boolean"
        ...
        : {}),
      ...(typeof parsed.rootAddress === "string"
        ? { rootAddress: parsed.rootAddress }
        : {}),
      ...(typeof parsed.host === "string" ? { host: parsed.host } : {}),
      ...(typeof parsed.port === "number" && Number.isFinite(parsed.port)
        ? { port: parsed.port }
        : {}),
      ...(typeof parsed.drumMidiDevice === "string"
        ? { drumMidiDevice: parsed.drumMidiDevice }
        : {}),
    };
  } catch {
    return null;
...
function mergeOscConfig(
  base: OscConfig,
  override: Partial<OscConfig>,
): OscConfig {
  return {
    ...base,
    ...override,
...
export function useSequencer() {
  ...
  const [lastOscDebugEvent, setLastOscDebugEvent] =
    useState<OscDebugEvent | null>(null);
  ...
  function handleMessage(msg: ServerMessage) {
    ...
    if (msg.type === "stab_update") {
      setStabs((prev) => {
        const nextStab: StabState = {
          ...
          mirrorOffEnabled: msg.mirrorOffEnabled,
          mirrorOffState: msg.mirrorOffState,
          x: msg.x,
          y: msg.y,
          cc3: msg.cc3,
          cc4: msg.cc4,
    ...
    if (msg.type === "osc_config_update") {
      ...
      if (
        pendingOscApplyRef.current &&
        equalOscConfig(msg.config, pendingOscApplyRef.current)
      ) {
        pendingOscApplyRef.current = null;
      }
    ...
    if (msg.type === "osc_debug_event") {
      ...
      setOsc((prev) => ({
        ...
        midiDevices: prev?.midiDevices ?? [],
        debugLog: [...(prev?.debugLog ?? []), msg.event].slice(
          -OSC_DEBUG_LOG_LIMIT,
        ),
      }));
    }
