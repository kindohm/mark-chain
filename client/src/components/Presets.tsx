 ...
 *
 * State persisted in browser localStorage, key: brunchh-preset-{0..15}
 */
...
const STORAGE_KEY = (i: number) => `brunchh-preset-${i}`;
...
export default function Presets({
  ...
  const recallPreset = (data: PresetData) => {
    ...
    sendMessage({
      ...
      numStates: data.chain.numStates,
    });
    for (let row = 0; row < data.chain.matrix.length; row++) {
      for (let col = 0; col < data.chain.matrix[row].length; col++) {
        sendMessage({
          ...
          row,
          col,
          value: data.chain.matrix[row][col],
        });
      }
    }
    ...
    // Anchor
    sendMessage({
      type: "set_anchor_enabled",
      isEnabled: data.anchor.isEnabled,
    });
    sendMessage({
      type: "set_anchor_division",
      division: data.anchor.division,
    });
    sendMessage({
      type: "set_anchor_midi",
      midiDevice: data.anchor.midiDevice,
      channel: data.anchor.channel,
    });
    ...
    data.stabs.forEach((s) => {
      const id = s.stabId;
      sendMessage({
        type: "set_stab_enabled",
        stabId: id,
        isEnabled: s.isEnabled,
      });
      sendMessage({
        type: "set_stab_num_steps",
        stabId: id,
        numSteps: s.numSteps,
      });
      sendMessage({
        ...
        {Array.from({ length: NUM_SLOTS }, (_, i) => {
          ...
          return (
            <button
              ...
              title={
                filled
                  ? `Slot ${i + 1}: short-press to recall, long-press to delete`
                  : `Slot ${i + 1}: long-press to save`
              }
